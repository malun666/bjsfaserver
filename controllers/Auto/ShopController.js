...
module.exports = {
   async getShop(ctx, next) {
    try {
      ...
      let sortObj = {};
      if (!ctx.query) {
        let shopList = await db.Shop.find({isDel: false}).exec();
        ctx.body = shopList;
        return;
      }
      ...
      // 處理排序
      let total = await db.Shop.find(queryObj).count().exec();

      let query = db.Shop.find(queryObj).sort(sortObj);
  ...
  async deleteShopById(ctx) {
    try {
      ...
      shop.isDel = 1;
      shop.save();
      ctx.body = { code: 1, msg: '删除成功', data: shop};
    } catch(e) {
  ...
  async updateShop(ctx) {
    try {
      ...
      shop.save();
      ctx.body = { code: 1, msg: '修改成功', data: shop};
    } catch(e) {
